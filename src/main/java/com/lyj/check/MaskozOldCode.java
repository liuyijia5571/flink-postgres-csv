...
public class MaskozOldCode {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String maskozFile = params.get("MASKOZ00_FILE");

        String masninCode = params.get("MASNIN_CODE");

        String maskaiCode = params.get("MASKAI_CODE");

        boolean checkParamsResult = checkParams(maskozFile, masninCode,maskaiCode);
        ...
        DataSource<String> maskozDs = env.readTextFile(maskozFile);
        ...
        DataSet<String> result = maskozDs.leftOuterJoin(masninDs).where(u -> {
            String[] split = u.split("\t");
            return split[4];
        ...
        result.writeAsText("result/maskoz.tsv", FileSystem.WriteMode.OVERWRITE);

        env.execute(MaskozOldCode.class.getName() + "_" + getFormattedDate());
    }
