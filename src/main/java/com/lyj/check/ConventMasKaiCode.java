 ...
 * 替换错误的code
 */
public class ConventMasKaiCode {

    private static final Logger logger = LoggerFactory.getLogger(ConventMasKaiCode.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataSet<Row> result = masKaiDs.leftOuterJoin(joinDs).where(row -> (String) row.getField(1)).equalTo(u -> u.f1)
                .with((first, second) -> {
                    if (second != null) {
                        if (first.getArity() > 2) {
                            String insertUserId = (String) first.getField(2);
                            if (insertUserId.contains("m04")||insertUserId.contains("m27")){
                                first.setField(1, second.f2);
                            }
                        } else {
                            first.setField(1, second.f2);
                        }
        ...
        env.execute(ConventMasKaiCode.class.getName() + "_" + getFormattedDate());
    }
